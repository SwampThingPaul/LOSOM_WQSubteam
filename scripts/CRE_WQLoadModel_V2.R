...
resid.val=S79.TP.mod$residuals
acf(resid.val)

acf.TPMod.rslt=data.frame()
for(h in 0:24){
  #demean
  ...
  acf.TPMod.rslt=rbind(acf.TPMod.rslt,data.frame(lag=h,estimate=as.numeric(stat$estimate),pval=stat$p.value))
}
acf.TPMod.rslt
...
with(acf.TPMod.rslt,points(lag,estimate,pch=21,bg=ifelse(pval<0.05,"indianred1","dodgerblue1"),lwd=0.01))
...
#with(mod.ecdf,lines(value,lwr.CI,lwd=1,lty=2,col="indianred1"))
#with(mod.ecdf,lines(value,upr.CI,lwd=1,lty=2,col="indianred1"))
with(dat.ecdf,shaded.range(value,lwr.CI,upr.CI,"dodgerblue1",lty=0,col.adj=0.25))
with(dat.ecdf,lines(value,proportion,lwd=2,col="dodgerblue1"))
#with(dat.ecdf,lines(value,lwr.CI,lwd=1,lty=2,col="dodgerblue1"))
#with(dat.ecdf,lines(value,upr.CI,lwd=1,lty=2,col="dodgerblue1"))
axis_fun(1,line=-0.5,xmaj,xmin,format(xmaj));axis_fun(2,ymaj,ymin,format(ymaj));box(lwd=1)
mtext(side=2,line=2.5,"Proportion")
mtext(side=1,line=1.5,"TP (\u03BCg L\u207B\u00B9)")
legend("topleft",c("Observed (\u00B1 95% CI)","Modelled (\u00B1 95% CI)"),
       lty=c(1,1),lwd=c(2,2),
...
axis_fun(1,line=-0.5,xmaj,xmin,format(xmaj));axis_fun(2,ymaj,ymin,format(ymaj));box(lwd=1)
mtext(side=2,line=2.5,"Proportion")
mtext(side=1,line=1.5,"TN (mg L\u207B\u00B9)")
legend("bottomright",c("Observed (\u00B1 95% CI)","Modelled (\u00B1 95% CI)"),
       lty=c(1,1),lwd=c(2,2),
       col=c("dodgerblue1","indianred1"),
       pt.cex=1.5,ncol=1,cex=0.75,bty="n",y.intersp=1,x.intersp=0.75,xpd=NA,xjust=0.5,yjust=0.5)
dev.off()
