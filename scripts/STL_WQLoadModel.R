...
q.stl.dat=data.frame()
for(i in 1:nrow(q.dbkeys)){
  tmp=DBHYDRO_daily(dates[1],dates[2],q.dbkeys$DBKEY[i])
  tmp$DBKEY=as.character(q.dbkeys$DBKEY[i])
  q.stl.dat=rbind(q.stl.dat,tmp)
  print(i)
}
q.stl.dat=merge(q.stl.dat,q.dbkeys,"DBKEY")
q.stl.dat$Date.EST=date.fun(q.stl.dat$Date)

#plot(Data.Value~Date.EST,subset(q.stl.dat,SITE=="S308"))
#plot(Data.Value~Date.EST,subset(q.stl.dat,SITE=="S80"))
#ddply(q.stl.dat,"SITE",summarise,min.val=min(Data.Value,na.rm=T))

q.stl.dat.xtab=cast(q.stl.dat,Date.EST~SITE,value="Data.Value",mean)
q.stl.dat.xtab$month=format(q.stl.dat.xtab$Date,"%m")
q.stl.dat.xtab$CY=format(q.stl.dat.xtab$Date,"%Y")
q.stl.dat.xtab$monCY=with(q.stl.dat.xtab,date.fun(paste(CY,month,"01",sep="-")))
q.stl.dat.xtab$WY=WY(q.stl.dat.xtab$Date.EST)

q.stl.dat.xtab$S308=with(q.stl.dat.xtab,ifelse(S308<0,0,S308))
q.stl.dat.xtab$C44.basin.in=with(q.stl.dat.xtab,ifelse(S80<S308,0,S80-S308))
q.stl.dat.xtab$basin.ratio=with(q.stl.dat.xtab,ifelse(S80==0,NA,C44.basin.in/S80))

q.stl.dat.xtab.mon=ddply(q.stl.dat.xtab,c("monCY","WY"),summarise,S308=sum(S308,na.rm=T),S80=sum(S80,na.rm=T))
q.stl.dat.xtab.mon$C44Basin=with(q.stl.dat.xtab.mon,ifelse(S80<S308,0,S80-S308))
q.stl.dat.xtab.mon$basin.q.ratio=with(q.stl.dat.xtab.mon,C44Basin/S80)

plot(C44Basin~monCY,q.stl.dat.xtab.mon,type="l")
plot(basin.q.ratio~monCY,q.stl.dat.xtab.mon,type="l")
plot(S308~monCY,q.stl.dat.xtab.mon,type="l")
with(q.stl.dat.xtab.mon,lines(monCY,S80,col="red"))

## No consistent/reliable stage data

stl.hydro.mon=q.stl.dat.xtab.mon

# STL WQ ------------------------------------------------------------------
params=data.frame(Test.Number=c(18,21,80,20,25,23,61,179,100),param=c("NOx","TKN","TN","NH4","TP","SRP","Chla","Chla","TOC"))
wq.dat=DBHYDRO_WQ(dates[1],dates[2],c("C44S80","S308C"),params$Test.Number)
...
wq.dat.xtab.mon=ddply(subset(wq.dat.xtab,Station.ID=="C44S80"),c("WY","monCY"),summarise,mean.TP=mean(TP,na.rm=T),mean.TN=mean(TN,na.rm=T),mean.TOC=mean(TOC,na.rm=T))
mean(wq.dat.xtab.mon$mean.TOC,na.rm=T)

plot(mean.TOC~format(wq.dat.xtab.mon$monCY,"%m"),wq.dat.xtab.mon)
wq.dat.xtab.mon.TOC=ddply(subset(wq.dat.xtab,Station.ID=="C44S80"),c("month"),summarise,POR.mean.TOC=mean(TOC,na.rm=T))
...
#tiff(filename=paste0(plot.path,"S80_MonthlyTPTN.tiff"),width=4.75,height=6,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
#png(filename=paste0(plot.path,"png/S80_MonthlyTPTN.png"),width=4.75,height=6,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,3,0.75,0.75),oma=c(2,1,0.5,0.5));
layout(matrix(1:2,2,1))

ylim.val=c(0.05,0.6);ymaj=c(0.05,log.scale.fun(ylim.val,"major"),0.5);ymin=log.scale.fun(ylim.val,"minor")
xlim.val=c(1,12);xmaj=1:12;xmaj.lab=c(5:12,1:4)
plot(mean.TP~month.plot,stl.hydro.wq.mon,log="y",ylim=ylim.val,xlim=xlim.val,type="n",yaxs="i",axes=F,ylab=NA,xlab=NA)
abline(h=ymaj,v=xmaj,lty=3,col="grey")
with(stl.hydro.wq.mon,points(month.plot,mean.TP,pch=21,bg=adjustcolor("dodgerblue1",0.25),col="dodgerblue1",lwd=0.1,cex=1.25))
k=predict(loess(mean.TP~month.plot,stl.hydro.wq.mon),data.frame(month.plot=seq(1,12,length.out = 24)),se=T)
...
plot(mean.TN~month.plot,stl.hydro.wq.mon,ylim=ylim.val,xlim=xlim.val,type="n",yaxs="i",axes=F,ylab=NA,xlab=NA)
abline(h=ymaj,v=xmaj,lty=3,col="grey")
with(stl.hydro.wq.mon,points(month.plot,mean.TN,pch=21,bg=adjustcolor("dodgerblue1",0.25),col="dodgerblue1",lwd=0.1,cex=1.25))
k=predict(loess(mean.TN~month.plot,stl.hydro.wq.mon),data.frame(month.plot=seq(1,12,length.out = 24)),se=T)
lines(seq(1,12,length.out = 24),k$fit,lwd=2,col="red")
lines(seq(1,12,length.out = 24),k$fit - qt(0.975,k$df)*k$se, lty=2,lwd=1,col="red")
lines(seq(1,12,length.out = 24),k$fit + qt(0.975,k$df)*k$se, lty=2,lwd=1,col="red")
...
#tiff(filename=paste0(plot.path,"S80_TPACF.tiff"),width=5,height=3.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
#png(filename=paste0(plot.path,"png/S80_TPACF.png"),width=5,height=4,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,3,0.75,0.75),oma=c(2,1,0.5,0.5));
...
ylim.val=c(0.05,0.6);ymaj=log.scale.fun(ylim.val,"major");ymin=log.scale.fun(ylim.val,"minor")#by.y=0.05;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
...
#tiff(filename=paste0(plot.path,"S80_lagplots.tiff"),width=7,height=2.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
#png(filename=paste0(plot.path,"png/S80_lagplots.png"),width=7,height=2.5,units="in",res=200,type="windows",bg="white")
...
xlim.val=c(0,24);by.x=5;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/by.x)
#tiff(filename=paste0(plot.path,"S80_TNACF.tiff"),width=5,height=3.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
#png(filename=paste0(plot.path,"png/S80_TNACF.png"),width=5,height=4,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,3,0.75,0.75),oma=c(2,1,0.5,0.5));
...
xlim.val=ylim.val;by.x=1;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/by.x)
#tiff(filename=paste0(plot.path,"S80_TN_lagplots.tiff"),width=7.5,height=2.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
#png(filename=paste0(plot.path,"png/S80_TN_lagplots.png"),width=7,height=2.5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,2,0.75,0.75),oma=c(3,1.5,0.5,0.5));
layout(matrix(1:5,1,5))
for(h in 0:4){
  tmp.dat=data.frame(dat=as.zoo(subset(wq.dat.xtab.mon,is.na(mean.TN)==F)$mean.TN),lag=lag(as.zoo(subset(wq.dat.xtab.mon,is.na(mean.TN)==F)$mean.TN),-h,na.pad=T))
  ...
  if(h==0){mtext(side=2,line=2,"TN (mg L\u207B\u00B9)",cex=1)}
  axis_fun(1,line=-0.5,xmaj,xmin,format(xmaj),cex=1);box(lwd=1)
  mtext(side=3,line=0.2,paste0("Lag ",h),cex=0.75)
  
  stat=cor.test(tmp.dat$lag,tmp.dat$dat)
  est.val=round(as.numeric(stat$estimate),2)
  pval.val=ifelse(stat$p.value<0.01,"< 0.01",paste0(" = ",round(stat$p.value,2)))
  text.val=as.expression(paste("r"["Pearson"]," = ",est.val,"; \u03C1  = ",pval.val,sep=""))
  text.val=bquote(paste(r[Pearson]==.(est.val)~"; ",rho~.(pval.val)))
  legend("bottomright",legend=text.val,lty=NA,lwd=NA,col=NA,cex=0.75,ncol=1,bty="n",y.intersp=1,x.intersp=0.75,xpd=NA,xjust=0.5,yjust=0.5)
...
# Annual
wq.dat.xtab.WY=ddply(subset(wq.dat.xtab,Station.ID=="C44S80"),c("WY"),summarise,mean.TP=mean(TP,na.rm=T),N.TP=N(TP),mean.TN=mean(TN,na.rm=T),N.TN=N(TN))
...
#tiff(filename=paste0(plot.path,"S80_MonthlyTPTN_TS.tiff"),width=5,height=6,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
par(family="serif",mar=c(1,3,0.75,0.75),oma=c(2,1,0.5,0.5));
layout(matrix(1:2,2,1))
ylim.val=c(0.05,1);ymaj=c(0.05,log.scale.fun(ylim.val,"major"),0.5);ymin=log.scale.fun(ylim.val,"minor")
...
#tiff(filename=paste0(plot.path,"S80_MonthlyTP_hist.tiff"),width=3,height=6.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
par(family="serif",mar=c(0.25,3,0.75,2),oma=c(4,1,0.5,0.5));
layout(matrix(1:12,12,1))

xlim.val=c(0.06,0.6);by.x=0.05;xmaj=seq(min(0,xlim.val[1]),xlim.val[2],by.x);xmin=seq(min(0,xlim.val[1]),xlim.val[2],by.x/2)
ylim.val=c(0,8);by.y=4;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
for(i in 1:12){
  ...
  if(i==12){axis_fun(1,line=-0.5,xmaj,xmin,format(xmaj*1000))}else(axis_fun(1,xmaj,xmin,NA))
...
#tiff(filename=paste0(plot.path,"S80_MonthlyTPWY.tiff"),width=3,height=6.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
...
#tiff(filename=paste0(plot.path,"S80_MonthlyTNWY.tiff"),width=3,height=6.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
par(family="serif",mar=c(0.25,3.5,0.75,1.75),oma=c(4,1,0.5,0.5));
layout(matrix(1:12,12,1))
...
###
## TP Model
vars=c("mean.TP","C44Basin","S308","basin.q.ratio","hydro.season")
#S80.TP.mod=lm(log(mean.TP)~.,na.omit(stl.hydro.wq.mon[,vars]))
S80.TP.mod=lm(log(mean.TP)~C44Basin+S308+basin.q.ratio+hydro.season,na.omit(stl.hydro.wq.mon[,vars]))
...
#AIC model
S80.TP.mod.sw=stepAIC(S80.TP.mod,direction="both",trace=F)
S80.TP.mod.sw$anova

#tiff(filename=paste0(plot.path,"S80_TPModel_diag.tiff"),width=6,height=5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
...
xlim.val=c(-2.25,-1.0);by.x=0.5;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/2)
plot(S80.TP.mod$fitted.values,S80.TP.mod$residuals,ylim=ylim.val,xlim=xlim.val,type="n",axes=F,ylab=NA,xlab=NA)
abline(h=ymaj,v=xmaj,lty=3,col="grey80")
abline(h=0)
with(S80.TP.mod,points(fitted.values,residuals,pch=21,bg=adjustcolor(cols,0.5),col="grey",lwd=0.1,cex=1.25))
fit.vals=S80.TP.mod$fitted.values
res.vals=S80.TP.mod$residuals
with(lowess(fit.vals,res.vals),lines(x,y,col="red",lwd=2))
axis_fun(1,line=-0.5,xmaj,xmin,format(xmaj));axis_fun(2,ymaj,ymin,format(round(ymaj,2)));box(lwd=1)
...
rstd=rstandard(S80.TP.mod)
qq.x=qq.function(S80.TP.mod$residuals)
...
ylim.val=c(-3,3);by.y=1;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
xlim.val=c(0,0.5);by.x=0.2;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/2)
lev=hatvalues(S80.TP.mod)
...
S80.TP.pred.mod=with(na.omit(stl.hydro.wq.mon[,vars]),exp(predict(S80.TP.mod,data.frame(C44Basin=C44Basin,S308=S308,basin.q.ratio=basin.q.ratio,hydro.season=hydro.season),interval="confidence")))

delta=na.omit(stl.hydro.wq.mon[,vars])$mean.TP-S80.TP.pred.mod[,1]
mod.d=density(na.omit(as.numeric(S80.TP.pred.mod[,1])))
samp.d=density(stl.hydro.wq.mon$mean.TP)
mod.compare=data.frame(dat=c(S80.TP.pred.mod[,1],subset(stl.hydro.wq.mon,is.na(mean.TP)==F)$mean.TP),group=c(rep("mod",length(S80.TP.pred.mod[,1])),rep("obs",length(subset(stl.hydro.wq.mon,is.na(mean.TP)==F)$mean.TP))))
kruskal.test(dat~group,mod.compare)

ylim.val=c(0.05,0.5);ymaj=c(0.05,log.scale.fun(ylim.val,"major"),0.5);ymin=log.scale.fun(ylim.val,"minor")
#tiff(filename=paste0(plot.path,"S80_TPModel_Obs.tiff"),width=3.5,height=3,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
...
#tiff(filename=paste0(plot.path,"S80_TPModel_compare.tiff"),width=6,height=4,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
...
plot(mean.TP~S80.TP.pred.mod[,1],na.omit(stl.hydro.wq.mon[,vars]),ylim=ylim.val,xlim=xlim.val,type="n",log="xy",axes=F,ylab=NA,xlab=NA)
abline(h=ymaj,v=xmaj,lty=3,col="grey")
with(na.omit(stl.hydro.wq.mon[,vars]),points(S80.TP.pred.mod[,1],mean.TP,pch=21,bg=adjustcolor("forestgreen",0.25),lwd=0.1,col="grey"))
abline(0,1,lty=2)
...
ylim.val=c(-0.2,0.3);by.y=0.1;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
xlim.val=date.fun(dates);xmaj=seq(xlim.val[1],xlim.val[2],"4 years");xmin=seq(xlim.val[1],xlim.val[2],"1 years")
plot(mean.TP~monCY,na.omit(stl.hydro.wq.mon[,c("monCY",vars)]),ylim=ylim.val,xlim=xlim.val,type="n",axes=F,ylab=NA,xlab=NA)
abline(h=ymaj,v=xmaj,lty=3,col="grey")
abline(h=0)
segments(na.omit(stl.hydro.wq.mon[,c("monCY",vars)])$monCY,0,na.omit(stl.hydro.wq.mon[,c("monCY",vars)])$monCY,delta,lty=1,col="red")
points(na.omit(stl.hydro.wq.mon[,c("monCY",vars)])$monCY,delta,pch=21,bg="red",lwd=0.1)
...
ylim.val=c(0.05,0.5);ymaj=c(0.05,log.scale.fun(ylim.val,"major"),0.5);ymin=log.scale.fun(ylim.val,"minor")
xlim.val=date.fun(dates);xmaj=seq(xlim.val[1],xlim.val[2],"4 years");xmin=seq(xlim.val[1],xlim.val[2],"1 years")
#tiff(filename=paste0(plot.path,"S80_TPModel.tiff"),width=6,height=3.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
#png(filename=paste0(plot.path,"S80_TPModel.png"),width=6,height=3.5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(2,3,0.25,0.5),oma=c(2,1.75,0.75,0.75));

plot(mean.TP~monCY,na.omit(stl.hydro.wq.mon[,c("monCY",vars)]),ylim=ylim.val,xlim=xlim.val,log="y",type="n",axes=F,ylab=NA,xlab=NA)
abline(h=ymaj,v=xmaj,lty=3,col="grey")
#with(stl.hydro.wq.mon,pt_line(monCY,S80.TP.pred.mod,2,adjustcolor("indianred1",0.5),1,21,adjustcolor("indianred1",0.5),cex=1.25))
#shaded.range(stl.hydro.wq.mon$monCY,S80.TP.pred.mod[,2],S80.TP.pred.mod[,3],"indianred1",lty=1)
with(na.omit(stl.hydro.wq.mon[,c("monCY",vars)]),lines(monCY,S80.TP.pred.mod[,1],lty=1,col=adjustcolor("indianred1",0.5),lwd=2))
with(na.omit(stl.hydro.wq.mon[,c("monCY",vars)]),lines(monCY,S80.TP.pred.mod[,2],lty=2,col=adjustcolor("indianred1",0.5),lwd=1))
with(na.omit(stl.hydro.wq.mon[,c("monCY",vars)]),lines(monCY,S80.TP.pred.mod[,3],lty=2,col=adjustcolor("indianred1",0.5),lwd=1))
with(na.omit(stl.hydro.wq.mon[,c("monCY",vars)]),pt_line(monCY,mean.TP,2,adjustcolor("dodgerblue1",0.5),1,21,adjustcolor("dodgerblue1",0.5),cex=1))
...
#tiff(filename=paste0(plot.path,"S80_TPParameters.tiff"),width=6,height=5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
par(family="serif",mar=c(2,3.5,1.5,1.75),oma=c(2,1,0.5,0.5));
layout(matrix(1:4,2,2))
ylim.val=c(0.05,0.5);ymaj=log.scale.fun(ylim.val,"major");ymin=log.scale.fun(ylim.val,"minor")
xlim.val=c(5.5,8.6);by.x=1;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/2)

xlim.val=c(0,12e4);by.x=6e4;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/2)
plot(mean.TP~C44Basin,stl.hydro.wq.mon,log="y",ylim=ylim.val,xlim=xlim.val,type="n",axes=F,ylab=NA,xlab=NA)
abline(h=ymaj,v=xmaj,lty=3,col="grey")
with(stl.hydro.wq.mon,points(C44Basin,mean.TP,pch=21,bg="indianred1",lwd=0.1))
mod=lm(log(mean.TP)~C44Basin,stl.hydro.wq.mon)
x.val=seq(min(stl.hydro.wq.mon$C44Basin,na.rm=T),max(stl.hydro.wq.mon$C44Basin,na.rm=T),length.out=100)
mod.pred=predict(mod,data.frame(C44Basin=x.val),interval="confidence")
...
axis_fun(2,ymaj,ymin,ymaj*1000);box(lwd=1)
mtext(side=1,line=1.75,expression(paste("Q"["C44Basin"]," x10"^3,"; CFS")),cex=0.8)

xlim.val=c(0,15e4);by.x=5e4;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/2)
plot(mean.TP~S308,stl.hydro.wq.mon,log="y",ylim=ylim.val,xlim=xlim.val,type="n",axes=F,ylab=NA,xlab=NA)
abline(h=ymaj,v=xmaj,lty=3,col="grey")
with(stl.hydro.wq.mon,points(S308,mean.TP,pch=21,bg="indianred1",lwd=0.1))
mod=lm(log(mean.TP)~S308,stl.hydro.wq.mon)
x.val=seq(min(stl.hydro.wq.mon$S308,na.rm=T),max(stl.hydro.wq.mon$S308,na.rm=T),length.out=100)
mod.pred=predict(mod,data.frame(S308=x.val),interval="confidence")
shaded.range(x.val,exp(mod.pred[,2]),exp(mod.pred[,3]),"grey",lty=1)
lines(x.val,exp(mod.pred[,1]),lty=2)
axis_fun(1,line=-0.5,xmaj,xmin,xmaj/1e4)
axis_fun(2,ymaj,ymin,ymaj*1000);box(lwd=1)
mtext(side=1,line=2,expression(paste("Q"["S308"]," x10"^3,"; CFS")),cex=0.8)

xlim.val=c(0,1);by.x=0.2;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/2)
plot(mean.TP~basin.q.ratio,stl.hydro.wq.mon,log="y",ylim=ylim.val,xlim=xlim.val,type="n",axes=F,ylab=NA,xlab=NA)
abline(h=ymaj,v=xmaj,lty=3,col="grey")
with(stl.hydro.wq.mon,points(basin.q.ratio,mean.TP,pch=21,bg="indianred1",lwd=0.1))
mod=lm(log(mean.TP)~basin.q.ratio,stl.hydro.wq.mon)
x.val=seq(min(stl.hydro.wq.mon$basin.q.ratio,na.rm=T),max(stl.hydro.wq.mon$basin.q.ratio,na.rm=T),length.out=100)
mod.pred=predict(mod,data.frame(basin.q.ratio=x.val),interval="confidence")
shaded.range(x.val,exp(mod.pred[,2]),exp(mod.pred[,3]),"grey",lty=1)
lines(x.val,exp(mod.pred[,1]),lty=2)
axis_fun(1,line=-0.5,xmaj,xmin,format(xmaj))
axis_fun(2,ymaj,ymin,ymaj*1000);box(lwd=1)
mtext(side=1,line=2,expression(paste("Q"["C44Basin"],": Q"["S308"])),cex=0.8)

tmp=ddply(stl.hydro.wq.mon,"hydro.season",summarise,mean.val=mean(mean.TP,na.rm=T),SE.val=SE(mean.TP))
ylim.val=c(0.05,0.35);ymaj=log.scale.fun(ylim.val,"major");ymin=log.scale.fun(ylim.val,"minor")
#boxplot(mean.TP~hydro.season,stl.hydro.wq.mon)
...
# Grömping (2006) Relative Importance for Linear Regression in R: The Package relaimpo. Journal of Statistical Software. doi: 10.18637/jss.v017.i01

# Calculate Relative Importance for Each Predictor
library(relaimpo)
S80.TP.mod=lm(log(mean.TP)~C44Basin+S308+basin.q.ratio+as.numeric(hydro.season=='A_Wet'),na.omit(stl.hydro.wq.mon[,vars]))
calc.relimp(S80.TP.mod,type = c("lmg"), rela = TRUE)

# Bootstrap Measures of Relative Importance (1000 samples)
boot <- boot.relimp(S80.TP.mod, b = 1000, rank=T,
                    diff = TRUE, rela = TRUE)
booteval.relimp(boot) # print result
plot(booteval.relimp(boot,sort=TRUE))
#edit(plot.relimplmbooteval)

rslt=booteval.relimp(boot,sort=TRUE)
pick=which.max(rslt@level)
index <- sort(rslt@lmg, decreasing = T, index = T)$ix
xnames=rslt@namen[2:((length(test@namen) - 1) + 1)][index]
xlabs=c(expression(paste("Q"["C44Basin"])),expression(paste("Q"["S308"])),"Basin Ratio","Season")
ylim.val=c(0,1);by.y=0.2;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
#tiff(filename=paste0(plot.path,"S80_TPModel_relaimpo.tiff"),width=4,height=3,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
...
axis_fun(1,line=-0.5,x,x,xlabs[index],cex=0.75)
